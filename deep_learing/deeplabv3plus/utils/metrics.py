...
def _fast_hist(true, pred, num_classes, device):
    true = true.cpu()
    pred = pred.cpu()
...
def eval_metrics(true, pred, num_classes, device, cm=False):
    ...
    for t, p in zip(true, pred):
        hist += _fast_hist(t.flatten(), p.flatten(), num_classes, device)
    overall_acc = overall_pixel_accuracy(hist)
    avg_per_class_acc = per_class_pixel_accuracy(hist)
    avg_jacc = jaccard_index(hist)
    avg_dice = dice_coefficient(hist)
    if cm ==True:
        return overall_acc, avg_per_class_acc, avg_jacc, avg_dice, hist
    else:
